...
from protonets.data.utils import load_image_path, scale_image, convert_tensor, apply_data_augmentation


TIEREDIMAGENET_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../Data/tieredImagenet')
TIEREDIMAGENET_CACHE = { }

def load_class_images(d):
    if d['class'] not in TIEREDIMAGENET_CACHE:
        split, image_class = d['class'].split('/')
        image_dir = os.path.join(TIEREDIMAGENET_DATA_DIR, 'data', split, image_class)
        
        class_images = sorted(glob.glob(os.path.join(image_dir, '*.jpg')))
        if len(class_images) == 0:
            raise Exception("No images found for TIEREDIMAGENET class {} at {}. Did you run download_imagenet.sh first?".format(d['class'], image_dir))
        ...
        image_ds = TransformDataset(ListDataset(class_images),
                                    compose([partial(convert_dict, 'file_name'),
                                            ...
                                            #  partial(apply_data_augmentation, 'data')
                                             ]))

        loader = torch.utils.data.DataLoader(image_ds, batch_size=len(image_ds), shuffle=False)
        for sample in loader:
            TIEREDIMAGENET_CACHE[d['class']] = sample['data']
            break # only need one sample because batch size equal to dataset length
        
    return { 'class': d['class'], 'data': TIEREDIMAGENET_CACHE[d['class']] }
...
def load(opt, splits):
    split_dir = os.path.join(TIEREDIMAGENET_DATA_DIR, 'splits', opt['data.split'])
    ...
    for split in splits:
        ...
        with open(os.path.join(split_dir, "{:s}.txt".format(split)), 'r') as f:
            for class_name in f.readlines()[1:]:
                    class_name = split + '/' + class_name.rstrip('\n')
