...
def load_class_images(d):
    if d['class'] not in MINIIMAGENET_CACHE:
        ...
        image_dir = os.path.join(MINIIMAGENET_DATA_DIR, 'Data', split, image_class)
        
        class_images = sorted(glob.glob(os.path.join(image_dir, '*.jpg')))
        if len(class_images) == 0:
            raise Exception("No images found for miniImagenet class {} at {}. Did you run download_imagenet.sh first?".format(d['class'], image_dir))
    ...
    for split in splits:
        ...
        with open(os.path.join(split_dir, "{:s}.csv".format(split)), 'r') as f:
            for class_name in f.readlines()[1:]:
                    class_name = split + '/' + class_name.split(',')[1].rstrip('\n')
