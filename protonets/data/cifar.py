...
CIFAR_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../Data/cifar100')
CIFAR_CACHE = { }

def load_class_images(d):
    if d['class'] not in CIFAR_CACHE:
        image_class = d['class']
        image_dir = os.path.join(CIFAR_DATA_DIR, 'data', image_class) 
        class_images = sorted(glob.glob(os.path.join(image_dir, '*.png')))
        if len(class_images) == 0:
            raise Exception("No images found for CIFAR class {} at {}. Did you run download_imagenet.sh first?".format(d['class'], image_dir))
        ...
        for sample in loader:
            CIFAR_CACHE[d['class']] = sample['data']
            break # only need one sample because batch size equal to dataset length
        
    return { 'class': d['class'], 'data': CIFAR_CACHE[d['class']] }
...
def load(opt, splits):
    split_dir = os.path.join(CIFAR_DATA_DIR, 'splits', opt['data.split'])
