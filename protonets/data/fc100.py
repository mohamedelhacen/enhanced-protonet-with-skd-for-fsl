...
CIFAR100_DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../Data/fc100')
CIFAR100_CACHE = { }

def load_class_images(d):
    if d['class'] not in CIFAR100_CACHE:
        split, image_class = d['class'].split('/')
        image_dir = os.path.join(CIFAR100_DATA_DIR, split, image_class) 
        class_images = sorted(glob.glob(os.path.join(image_dir, '*.jpg')))
        if len(class_images) == 0:
            raise Exception("No images found for CIFAR100 class {} at {}. Did you run download_imagenet.sh first?".format(d['class'], image_dir))
    ...
    for split in splits:
        ...
        with open(os.path.join(split_dir, "{:s}.txt".format(split)), 'r') as f:
            for class_name in f.readlines()[1:]:
                    # print(class_name)
                    class_name = split + '/' + class_name.strip()
