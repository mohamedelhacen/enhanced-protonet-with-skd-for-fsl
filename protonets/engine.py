...
class Engine(object):
    ...
    def train(self, **kwargs):
        ...
        state['optimizer'] = state['optim_method'](state['model'].parameters(), **state['optim_config']) # , momentum=0.9
        ...
        if state['epoch'] == state['max_epoch']:
            state['model'].cpu()
            torch.save(state['model'], os.path.join(state['result'], 'last_model.pt'))
