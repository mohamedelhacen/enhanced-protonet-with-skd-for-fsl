...
def main(opt):
    ...
    trace_name = f"trace_{opt['data.dataset']}_{opt['data.way']}_{opt['data.shot']}_{opt['data.query']}_{opt['data.train_episodes']}_{opt['model.model_name']}.txt"

    trace_file = os.path.join(opt['log.exp_dir'], trace_name)
        ...
        if val_loader is not None:
            if meter_vals['val']['loss'] < hook_state['best_loss']:
                ...
                if hook_state['wait'] > opt['train.patience']:
                    state['model'].cpu()
                    torch.save(state['model'], os.path.join(opt['log.exp_dir'], 'last_model.pt'))
